...
export function ScanDashboard({ playlistId, venueuid }: ScanDashboardProps) {
  ...
  const handleScans = async () => {
      ...
      const newScan: Scan = {
        // uuid is optional and will be generated by the database
        timestamp: new Date().toISOString(), // Current time in ISO format
        attendees_uuid: '1', // Replace with actual attendee UUID
        gigs_uuid: 'gig_uuid_here',          // Replace with actual gig UUID
        venues_uuid: 'venue_uuid_here',      // Replace with actual venue UUID
    };
      try {
        // Assuming addAttendeeIfNotExists is also async

        // Now for the scan
        const scanResult = await addScan(newScan);
